...
if ($db->isInputMode($chat_id) === 'def' && !empty($data['message']['text'])) {
    switch ($command) {
        ...
        case $command !== null && strpos($command, 'Возобновить рассылку') === 0:
            if ($db->isAdmin($telegram, $chat_id)) { exec('/usr/bin/python /var/www/t.mercadalibre.live/src/broadcast_switch.py --start'); }
            break;
        case $command !== null && strpos($command, 'Остановить рассылку') === 0:
            if ($db->isAdmin($telegram, $chat_id)) { exec('/usr/bin/python /var/www/t.mercadalibre.live/src/broadcast_switch.py --stop'); }
            break;
